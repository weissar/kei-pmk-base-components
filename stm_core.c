...
bool Nucleo_SetPinGPIO(GPIO_TypeDef *gpio, uint32_t bitnum, eIoPortModes mode)
{
  ...
  uint32_t rstr_mask = 0;     // value fot xxRSTR register

  switch ((uint32_t) gpio)    // selected GPIO
  {
    case (uint32_t) GPIOA:
    ...
    case (uint32_t) GPIOE:
      ...
      break;
#if defined(STM32F411xE)   // 411RE hasn't GPIOF and G
#else
    case (uint32_t)GPIOF:
    ...
    case (uint32_t)GPIOG:
      ...
      break;
#endif    case (uint32_t) GPIOH:
      enr_mask = RCC_AHB1ENR_GPIOHEN;
      rstr_mask = RCC_AHB1RSTR_GPIOHRST;
      break;
  }

  if ((enr_mask == 0) || (rstr_mask == 0))  // GPIO not selected ?
    return false;                           // error sign

  if (!(RCC->AHB1ENR & enr_mask))           // not started yet ?
  {
    RCC->AHB1ENR |= enr_mask;               // enable clock
    RCC->AHB1RSTR |= rstr_mask;             // make reset
    RCC->AHB1RSTR &= ~rstr_mask;            // end block reset pulse
  }

  gpio->MODER &= ~(0x03 << (2 * bitnum));   // clean corresponding 2 bits in register
  gpio->PUPDR &= ~(0x03 << (2 * bitnum));   // clean 2 bits - 00 = no pu/pd
  gpio->OSPEEDR &= ~(0x03 << (2 * bitnum)); // dtto
  ...
  switch (mode)
  {
    ...
    case ioPortOutputPP:
      ...
      if (mode == ioPortOutputOC)               // required open collector (drain)
        gpio->OTYPER |= 0x01 << bitnum;         // 1 = OC/Open drain
      else
        gpio->OTYPER &= ~(0x01 << bitnum);      // 0 = push-pull
      break;
    case ioPortInputPU:                         // moder bits remains at 00 = input
    ...
    case ioPortAnalog:                          // 11 - analog mode
      ...
      break;
    case ioPortAlternatePP:                     // common case
    case ioPortAlternateOC:
      gpio->MODER |= 0x02 << (2 * bitnum);      // set bits 10 = AF
...
{
  gpio->AFR[bitnum / 8] &= ~(0x0f << (4 * (bitnum & 0x07)));
  gpio->AFR[bitnum / 8] |= ((afValue & 0x0f) << (4 * (bitnum & 0x07)));
  return true;
}
